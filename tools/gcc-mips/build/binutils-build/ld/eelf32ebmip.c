...
static void gldelf32ebmip_after_parse (void);
...
static lang_output_section_statement_type *gldelf32ebmip_place_orphan
...
static void
gldelf32ebmip_after_parse (void)
{
...
static void
gldelf32ebmip_append_to_separated_string (char **to, char *op_arg)
{
  if (*to == NULL)
    *to = xstrdup (op_arg);
  else
	...
	if (audit_libs && *audit_libs != '\0')
	  {
		...
		if (cp != NULL && *cp != '\0')
		  gldelf32ebmip_append_to_separated_string (&depaudit, cp);
...
OUTPUT_ARCH(mips)\n\
ENTRY(_start)\n\
SEARCH_DIR(\"/home/users/ktran20/lab3/tools/gcc-mips/mips_tc/mips-elf/lib\");\n\
SECTIONS\n\
{\n\
  /* Read-only sections, merged into text segment: */\n\
...
gldelf32ebmip_handle_option (int optc)
{
  switch (optc)
	...
	einfo (_("%P%F: invalid --compress-debug-sections option: `%s'\n"),
      ...
      break;
    case OPTION_AUDIT:
	gldelf32ebmip_append_to_separated_string (&audit, optarg);
	break;

    case 'P':
	gldelf32ebmip_append_to_separated_string (&depaudit, optarg);
	break;
